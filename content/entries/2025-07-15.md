...
4. Apply [gradient descent](https://en.wikipedia.org/wiki/Gradient_descent) to the objective function $| r(\delta) |_\infty = \max(|r(\delta)|)$

There is a bit of a problem with this approach, though. Although the risk $r(\delta)$ is a differentiable function with respect to the different components of the $\delta$ vector, the supremum norm $|\cdot|_\infty$ is *not* a differentiable function of its vector argument, so gradient descent cannot really be applied (in its usual form) to the objective function $\delta\mapsto | r(\delta) |_\infty$.
